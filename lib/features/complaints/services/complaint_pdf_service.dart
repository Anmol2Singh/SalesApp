...
class ComplaintPdfService {
  static Future<pw.MemoryImage?> _resolveSignatureImage(
      String? urlOrBase64) async {
  ...
  static Future<Uint8List> generateCompletionReportPdf(
      Complaint complaint) async {
    final pdf = pw.Document();
    final generatedByName = await PdfService.resolveCurrentUserName();

    final customerSignatureImage =
        await _resolveSignatureImage(complaint.customerSignatureUrl);
    final techSignatureImage =
        await _resolveSignatureImage(complaint.technicianSignatureUrl);
    ...
    try {
      ...
      if (allReplacedItems.isEmpty) {
        final cRes = await Supabase.instance.client
            .from('complaints')
            .select('replaced_items')
            .eq('id', complaint.id)
            .maybeSingle();
        final cItems = cRes?['replaced_items'] as List?;
        if (cItems != null) {
          for (final itm in cItems) {
            if (itm is Map) {
              allReplacedItems.add(Map<String, dynamic>.from(itm));
            }
          }
      ...
      if (allReplacedItems.isNotEmpty) {
        ...
        for (final itm in allReplacedItems) {
          final name = (itm['item_name'] ?? '').toString().trim();
          final war = (itm['is_warranty'] as bool?) ?? false;
          final key = '${name.toLowerCase()}_$war';
          if (mergedMap.containsKey(key)) {
            final cur = mergedMap[key]!;
            final oldQty = (cur['quantity'] as num?)?.toInt() ?? 1;
            final addQty = (itm['quantity'] as num?)?.toInt() ?? 1;
            final newQty = oldQty + addQty;
            final curTotal = (cur['total_price'] as num?)?.toDouble() ?? 0.0;
            final itmTotal = (itm['total_price'] as num?)?.toDouble() ?? 0.0;
            cur['quantity'] = newQty;
            cur['total_price'] = curTotal + itmTotal;
          } else {
            mergedMap[key] = Map<String, dynamic>.from(itm);
          }
        }
        allReplacedItems.clear();
        allReplacedItems.addAll(mergedMap.values);
      }
    } catch (_) {}
    ...
    try {
      ...
      if (tRes != null && tRes['template_config'] is Map) {
        ...
        if (cfg['company_name'] != null &&
            cfg['company_name'].toString().trim().isNotEmpty) {
          companyName = cfg['company_name'].toString().trim();
        }
        if (cfg['company_address'] != null &&
            cfg['company_address'].toString().trim().isNotEmpty) {
          companyAddress = cfg['company_address'].toString().trim();
        }
        if (cfg['company_phone'] != null &&
            cfg['company_phone'].toString().trim().isNotEmpty) {
          companyPhone = cfg['company_phone'].toString().trim();
        }
        if (cfg['company_email'] != null &&
            cfg['company_email'].toString().trim().isNotEmpty) {
          companyEmail = cfg['company_email'].toString().trim();
        }
        if (cfg['company_gst'] != null &&
            cfg['company_gst'].toString().trim().isNotEmpty) {
          companyGst = cfg['company_gst'].toString().trim();
        }
        if (cfg['logo_url'] != null &&
            cfg['logo_url'].toString().trim().isNotEmpty) {
          try {
    ...
    if (formattedProductName.length > 25 &&
        RegExp(r'^[0-9a-fA-F\-]+$').hasMatch(formattedProductName.trim())) {
          ...
          return [
            ...
            pw.Container(
              ...
              child: pw.Column(
                crossAxisAlignment: pw.CrossAxisAlignment.start,
                children: [
                  pw.Text('CUSTOMER & SERVICE DETAILS',
                      style: pw.TextStyle(
            ...
            pw.Container(
              ...
              child: pw.Column(
                crossAxisAlignment: pw.CrossAxisAlignment.start,
                children: [
                  pw.Text('REPORTED ISSUE & RESOLUTION DETAILS',
                      style: pw.TextStyle(
                          fontSize: 12, fontWeight: pw.FontWeight.bold)),
                  pw.Divider(),
                  pw.SizedBox(height: 6),
                  pw.Text('Category / Fault: ${complaint.title}',
                      style: pw.TextStyle(
                          fontWeight: pw.FontWeight.bold, fontSize: 11)),
                  pw.SizedBox(height: 4),
                  pw.Text(
                      'Description: ${complaint.description.isNotEmpty ? complaint.description : "Service and repair completed."}',
            ...
            if (allReplacedItems.isNotEmpty) ...[
              pw.Container(
                ...
                child: pw.Column(
                  crossAxisAlignment: pw.CrossAxisAlignment.start,
                  children: [
                    pw.Text('REPLACED SPARE PARTS & ORDER ITEMS',
                        style: pw.TextStyle(
                            fontSize: 11,
                            fontWeight: pw.FontWeight.bold,
                            color: PdfColor.fromInt(0xFF1E1B4B))),
                        ...
                        ...allReplacedItems.map((item) {
                          ...
                          double total =
                              (item['total_price'] as num?)?.toDouble() ??
                                  ...
                                  (unitPrice * qty);
                          if (unitPrice == 0.0 && total > 0.0 && qty > 0) {
                            unitPrice = total / qty;
                          } else if (total == 0.0 && unitPrice > 0.0) {
                            total = unitPrice * qty;
                          }
                          if (unitPrice == 0.0 &&
                              total == 0.0 &&
                            ...
                            total = totalPaidAmount /
                                (allReplacedItems.isEmpty
                                    ? 1
                                    : allReplacedItems.length);
                            unitPrice = total / (qty > 0 ? qty : 1);
                          }
                          final inWarranty = (item['is_warranty'] == true) &&
                              unitPrice == 0.0 &&
                              total == 0.0;
                          return pw.TableRow(
                            children: [
              ...
              child: pw.Row(
                ...
                children: [
                  pw.Column(
                    ...
                    children: [
                      pw.Text(
                          'PAYMENT & TRANSACTION STATUS: ${hasSuccessfulPayment ? "PAYMENT SUCCESSFUL" : (isCoveredUnderWarranty ? "COVERED UNDER WARRANTY / AMC (FREE)" : "NO CHARGES")}',
                          style: pw.TextStyle(
                              fontSize: 9.5,
                              fontWeight: pw.FontWeight.bold,
                              color: PdfColors.green900)),
                      if (paymentRef != null && paymentRef.isNotEmpty)
                        pw.Text('Txn Ref: $paymentRef',
                            style: const pw.TextStyle(
                                fontSize: 8, color: PdfColors.grey700)),
                      if (paymentDate != null)
                        pw.Text(
                            'Paid Date: ${DateFormat('dd/MM/yyyy HH:mm').format(paymentDate)}',
                            style: const pw.TextStyle(
                                fontSize: 8, color: PdfColors.grey700)),
                    ],
                  ),
                  pw.Text(
                      'Total Paid: Rs. ${totalPaidAmount.toStringAsFixed(0)}',
                      style: pw.TextStyle(
                          fontSize: 11,
                          fontWeight: pw.FontWeight.bold,
                          color: PdfColors.green900)),
            ...
            pw.Row(
              ...
              children: [
                pw.Column(
                  ...
                  children: [
                    pw.Text(
                        'Technician: ${complaint.technicianName ?? "Field Staff"}',
                        style: pw.TextStyle(
                    ...
                    if (techSignatureImage != null)
                      pw.Container(
                        width: 130,
                        height: 50,
                        child: pw.Image(techSignatureImage,
                  ...
                  children: [
                    ...
                    if (customerSignatureImage != null)
                      pw.Container(
                        width: 130,
                        height: 50,
                        child: pw.Image(customerSignatureImage,
                            fit: pw.BoxFit.contain),
                      )
                    else
                      pw.Container(
            ...
            pw.Row(
              mainAxisAlignment: pw.MainAxisAlignment.spaceBetween,
              children: [
                pw.Text('Insiya Solar Industry', style: const pw.TextStyle(fontSize: 8, color: PdfColors.grey600)),
  ...
  static Future<void> shareCompletionReport(Complaint complaint) async {
    ...
    final safeTicket =
        complaint.ticketNumber.replaceAll(RegExp(r'[/\\?%*:|"<>]'), '_');
